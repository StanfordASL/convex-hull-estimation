...
def area_spherical_cap_two_spheres(d, r1, r2):
    # Note: this function assumes two spheres in 3d and returns 
    # the surface area of the spherical cap on the sphere of radius r1
    # see https://en.wikipedia.org/wiki/Spherical_cap

    # d: distance between the centers of the two balls
    # r1, r2: radius of each sphere
    vol_cap_1 = 2.0 * np.pi * (r1**2) * (
        1.0 + (r2**2 - r1**2 - d**2) / (2.0 * r1 * d)
        )
    return vol_cap_1

def length_spherical_cap_two_circles(d, r1, r2):
    # Note: this function assumes two circles in 2d and returns 
    # the length of the spherical cap on the circle of radius r1
    # see https://en.wikipedia.org/wiki/Spherical_cap

    # d: distance between the centers of the two balls
    # r1, r2: radius of each sphere

    # polar angle of the spherical cap on the circle of radius r1
    cos_theta = (r1**2 - r2**2 + d**2) / (2.0 * r1 * d)
    theta = np.arccos(cos_theta)

    # Note: factor two by definition of theta in 
    # https://en.wikipedia.org/wiki/Spherical_cap
    arc_length = 2 * theta * r1
    return arc_length